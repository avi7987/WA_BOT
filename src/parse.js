...
export function parseCommand(raw) {
  ...
  // "תייצר זימון ל..." / "זימון ל..." / "תוסיף ליומן ..." / "תקבע ביומן ..."
  // חשוב שיעבוד בלי AI: זו מילת מפתח מפורשת, ואם רק ה-LLM היה מזהה
  // אותה — כל האטה אצל הספק הייתה הופכת זימון למשימה.
  m = /^(?:תייצר|צור|תיצור|תקבע|קבע|תוסיף|הוסף)?\s*(?:לי\s+)?(?:זימון|אירוע)\s*(?:ל|ב)?\s*(.+)$/.exec(t);
  if (m && /זימון|אירוע/.test(t)) return { kind: 'event_create', text: m[1].trim() };

  m = /^(?:תוסיף|הוסף|תקבע|קבע)\s+ל?יומן\s*(?:ל|את)?\s*(.+)$/.exec(t);
  if (m) return { kind: 'event_create', text: m[1].trim() };
